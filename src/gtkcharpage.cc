...
void
GtkCharPage::send_notification_mail (void)
{
  if (!this->training->valid)
    return;

  /* Get and check address. */
  std::string address = **Config::conf.get_value("notifications.mail_address");

  if (address.empty())
  {
    this->popup_error_dialog("Mailing Error", "Receiver address not specified!",
         "Bad receiver address specified, check the configuration.");
    return;
  }

  /* Check if email notification need to be done (check minimum SP). */
  int minsp = 0;
  ...
  ApiSkillTreePtr tree = ApiSkillTree::request();
  int training_id = this->training->skill;
  ApiSkill const* skill = tree->get_skill_for_id(training_id);
  int to_level = this->training->to_level;

  if (skill == 0)
  {
    this->popup_error_dialog("Mailing Error", "Invalid skill information",
        "The skill in training is invalid and this can be considered "
        "as an internal error and should be reported.");
    return;
  }
  ...
  /* Simply return if there is no need to send a mail. */
  if (diff_sp < minsp)
  {
    std::cout << "Minimum SP: " << minsp << ", skill SP: " << diff_sp
        << ", NOT sending a mail!" << std::endl;
    return;
  }
  else
  {
    std::cout << "Minimum SP: " << minsp << ", skill SP: " << diff_sp
        << ", sending a mail!" << std::endl;
  }

  /* Setup the subject and the message. */
  std::string subject = this->get_char_name() + " has finished "
      + this->training_label.get_text();

  std::string message("Congratulations! " + subject + ".\n\n"
      "Statistics about the skill:\n"
      "Skill name: " + skill->name + "\n"
      "Acquired level: " + Helpers::get_string_from_int(to_level) + "\n"
      "Start SP: " + Helpers::get_dotted_str_from_int(start_sp) + "\n"
      "Destination SP: " + Helpers::get_dotted_str_from_int(dest_sp) + "\n"
      "Acquired SP: " + Helpers::get_dotted_str_from_int(diff_sp) + "\n"
      "SP/h: " + Helpers::get_string_from_int(spph) + "\n"
      "Total training time: " + EveTime::get_string_for_timediff
          ((time_t)(diff_sp / spps), false) + "\n\n"
      "This message was generated by GtkEveMon.\n");

  int ret = Mailer::send(address, subject, message);
  if (ret != 0)
  {
    this->popup_error_dialog("Mailing Error", "Mail could not be sent!",
        "The mailer program returned a non-zero exit code.");
    return;
  }
}
...
GtkCharPage::on_skill_completed (void)
{
  ...
  ConfValuePtr send_email = Config::conf.get_value
